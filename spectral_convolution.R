...
oli_path = file.path(basepath, "Ball_BA_RSR.v1.1-1.xlsx")
oli_fwhm = file.path(basepath, "Landsat_OLI_OLI2_FWHM.csv")
# Landsat full width half maximum (fwhm (nm) )
...
## Data formatted for Convolution
ls8 <- read.csv(file.path(basepath, "OLI_RSR.csv"))
ls9 <- read.csv(file.path(basepath, "OLI2_RSR.csv"))
# hyperspectral
hyper <- read.csv(file.path(lookuppath, "Lookup_long_nm.csv"))
head(hyper)

# RUN & SAVE - DO NOT RUN - data already exists
...
## READ in DATA
df_oli_grain_size = read.csv( file.path(resultpath, "modeled_gz_oli.csv") ) # LANDSAT 8
df_oli2_grain_size = read.csv( file.path(resultpath, "modeled_gz_oli2.csv") ) # LANDSAT 9
...
ndgsi_oli2 = df_oli2_grain_size %>% group_by(grain_size) %>% 
  summarize(ndgsi = (NIR_5 - SWIR1_6) / (NIR_5 + SWIR1_6))
# DO NOT SAVE - files already exist
# write.csv(ndgsi_oli, "ndgsi_oli.csv", row.names = F)
# write.csv(ndgsi_oli2, "ndgsi_oli2.csv", row.names = F)


## Reclassify Landsat imagery based on lookup table
# CHANGE TABLE TO SHOW MIN/MAX VALUES
#   this can be used in the reclassify tool
# e.g. 
# MIN | MAX | Val
# 30  | 31  | 0.....
...
## Lookup table plots
dfl <- read.csv(file.path(lookuppath, "LookupTable2.csv") )
head(dfl)
dfl2 = dfl %>%
...
{readxl::read_xlsx(oli9_path, sheet = x) %>%
    select(1:3) %>%
    rename(rsr = `BA RSR [watts]`,
